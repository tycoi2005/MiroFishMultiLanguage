...
def get_project(project_id: str):
    ...
    if not project:
        return jsonify(
            {
                "success": False,
                "error": msg("project_not_found", get_request_locale(), id=project_id),
            }
        ), 404
...
def delete_project(project_id: str):
    ...
    return jsonify(
        {"success": True, "message": msg("project_deleted", get_request_locale())}
    )
...
def reset_project(project_id: str):
    ...
    return jsonify(
        {
            "success": True,
            "message": msg("project_reset", get_request_locale()),
    ...
    try:
        ...
        if not simulation_requirement:
            return jsonify(
                {
                    "success": False,
                    "error": msg("provide_sim_requirement", get_request_locale()),
                }
            ), 400
        ...
        if not uploaded_files or all(not f.filename for f in uploaded_files):
            return jsonify(
                {
                    "success": False,
                    "error": msg("upload_at_least_one", get_request_locale()),
                }
            ), 400
        ...
        if not document_texts:
            ...
            return jsonify(
                {
                    "success": False,
                    "error": msg("no_docs_processed", get_request_locale()),
                }
            ), 400
    ...
    try:
            ...
            return jsonify(
                {
                    "success": False,
                    "error": msg(
                        "config_error", get_request_locale(), detail="; ".join(errors)
                    ),
                }
            ), 500
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify(
                {
                    "success": False,
                    "error": msg("sim_running_stop_first", get_request_locale()),
        ...
        ontology = project.ontology
        if not ontology:
            return jsonify({"success": False, "error": "Ontology not found"}), 400
