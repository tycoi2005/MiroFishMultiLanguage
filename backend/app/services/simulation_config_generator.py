...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        report_progress(
            1,
            "Generating time configuration..." if locale != "zh" else "生成时间配置...",
        )
        num_entities = len(entities)
        time_config_result = self._generate_time_config(
            context, num_entities, locale=locale
        )
        ...
        report_progress(
            2,
            "Generating event configuration and hot topics..."
            if locale != "zh"
            else "生成事件配置和热点话题...",
        )
        event_config_result = self._generate_event_config(
            context, simulation_requirement, entities, locale=locale
        )
        ...
        for batch_idx in range(num_batches):
            ...
            report_progress(
                ...
                (
                    ...
                    if locale != "zh"
                    else f"生成Agent配置 ({start_idx + 1}-{end_idx}/{len(entities)})..."
                ),
            )
    ...
    def _call_llm_with_retry(self, prompt: str, system_prompt: str) -> Dict[str, Any]:
        """LLM调用，使用LLMClient.chat_json()内置的重试和JSON修复逻辑"""
        messages = [
    ...
    def _generate_time_config(
        self, context: str, num_entities: int, locale: str = "en"
    ) -> Dict[str, Any]:
        """生成时间配置"""
        ...
        prompt = p.TIME_CONFIG_USER_PROMPT_TEMPLATE.format(
            context_truncated=context_truncated,
            max_agents_allowed=max_agents_allowed,
        )

        system_prompt = p.TIME_CONFIG_SYSTEM_PROMPT
    ...
    def _generate_event_config(
        ...
        prompt = p.EVENT_CONFIG_USER_PROMPT_TEMPLATE.format(
            simulation_requirement=simulation_requirement,
            context_truncated=context_truncated,
            type_info=type_info,
        )

        system_prompt = p.EVENT_CONFIG_SYSTEM_PROMPT
    ...
    def _generate_agent_configs_batch(
        ...
        prompt = p.AGENT_CONFIG_USER_PROMPT_TEMPLATE.format(
            simulation_requirement=simulation_requirement,
            entity_list_json=json.dumps(entity_list, ensure_ascii=False, indent=2),
        )

        system_prompt = p.AGENT_CONFIG_SYSTEM_PROMPT
