...
class ReportLogger:
    ...
    def log_planning_context(self, context: Dict[str, Any]):
        ...
        self.log(
            ...
            stage="planning",
            details={"message": "Fetching simulation context", "context": context},
        )
...
# ═══════════════════════════════════════════════════════════════
# Prompt constants moved to app/prompts/{en,zh}.py
# Use: p = get_prompts(locale); p.CONSTANT_NAME
# ═══════════════════════════════════════════════════════════════
...
class ReportAgent:
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "insight_forge": {
                "name": "insight_forge",
                "description": p.TOOL_DESC_INSIGHT_FORGE,
                "parameters": {
                    "query": p.TOOL_PARAM_INSIGHT_QUERY,
                    "report_context": p.TOOL_PARAM_INSIGHT_CONTEXT,
                },
            },
            "panorama_search": {
                "name": "panorama_search",
                "description": p.TOOL_DESC_PANORAMA_SEARCH,
                "parameters": {
                    "query": p.TOOL_PARAM_PANORAMA_QUERY,
                    "include_expired": getattr(
                        p,
            ...
            "quick_search": {
                "name": "quick_search",
                "description": p.TOOL_DESC_QUICK_SEARCH,
                "parameters": {
                    "query": getattr(
                        p, "TOOL_PARAM_QUICK_SEARCH_QUERY", "Search query string"
                    ),
                    "limit": getattr(
                        p,
            ...
            "interview_agents": {
                ...
                "description": p.TOOL_DESC_INTERVIEW_AGENTS,
                "parameters": {
                    "interview_topic": p.TOOL_PARAM_INTERVIEW_TOPIC,
                    "max_agents": p.TOOL_PARAM_INTERVIEW_COUNT,
                },
            },
        }
    ...
    def _execute_tool(
        ...
        try:
            if tool_name == "insight_forge":
                ...
                return result.to_text(locale=self.locale)
            ...
            # ========== 向后兼容的旧工具（内部重定向到新工具） ==========
                ...
                return (
                    ...
                    if self.locale != "zh"
                    else f"未知工具: {tool_name}。请使用以下工具之一: insight_forge, panorama_search, quick_search"
                )
    ...
    def _get_tools_description(self) -> str:
        ...
        for name, tool in self.tools.items():
            ...
            if params_desc:
                desc_parts.append(f"  {p.TOOLS_PARAMS_LABEL} {params_desc}")
        return "\n".join(desc_parts)
    ...
    def plan_outline(
        ...
        try:
            ...
            for item in p.FALLBACK_SECTIONS:
                ...
                fallback_sections.append(ReportSection(title=title))
            return ReportOutline(
                title=p.FALLBACK_REPORT_TITLE,
                summary=p.FALLBACK_REPORT_SUMMARY,
        ...
        for iteration in range(max_iterations):
            if progress_callback:
                progress_callback(
                    ...
                    (
                        f"Deep retrieval & writing ({tool_calls_count}/{self.MAX_TOOL_CALLS_PER_SECTION})"
                        if self.locale != "zh"
                        else f"深度检索与撰写中 ({tool_calls_count}/{self.MAX_TOOL_CALLS_PER_SECTION})"
                    ),
                )
                ...
                if tool_calls_count < min_tool_calls:
                    ...
                    unused_hint = (
                        (
                            ...
                            if self.locale != "zh"
                            else f"（这些工具还未使用，推荐用一下他们: {', '.join(unused_tools)}）"
                        )
                        if unused_tools
                        else ""
                    )
                    messages.append(
                        {
                            "role": "user",
                            "content": p.REACT_INSUFFICIENT_TOOLS_MSG.format(
                                tool_calls_count=tool_calls_count,
            ...
            if has_tool_calls:
                ...
                if tool_calls_count >= self.MAX_TOOL_CALLS_PER_SECTION:
                    ...
                    messages.append(
                        {
                            "role": "user",
                            "content": p.REACT_TOOL_LIMIT_MSG.format(
                                tool_calls_count=tool_calls_count,
                ...
                if len(result) > self.MAX_TOOL_RESULT_CHARS:
                    ...
                    truncation_note = (
                        ...
                        if self.locale != "zh"
                        else f"\n\n... [结果已截断，原始长度 {len(result)} 字符，已保留前 {self.MAX_TOOL_RESULT_CHARS} 字符]"
                    )
                    result = result[: self.MAX_TOOL_RESULT_CHARS] + truncation_note

                # 构建未使用工具提示
                unused_tools = all_tools - used_tools
                unused_hint = ""
                if unused_tools and tool_calls_count < self.MAX_TOOL_CALLS_PER_SECTION:
                    unused_hint = p.REACT_UNUSED_TOOLS_HINT.format(
                ...
                messages.append(
                    {
                        "role": "user",
                        "content": p.REACT_OBSERVATION_TEMPLATE.format(
                            tool_name=call["name"],
                            result=result,
                            tool_calls_count=tool_calls_count,
                            max_tool_calls=self.MAX_TOOL_CALLS_PER_SECTION,
            ...
            if tool_calls_count < min_tool_calls:
                ...
                messages.append(
                    {
                        "role": "user",
                        "content": p.REACT_INSUFFICIENT_TOOLS_MSG_ALT.format(
                            tool_calls_count=tool_calls_count,
                            min_tool_calls=min_tool_calls,
                            unused_hint=unused_hint,
                        ),
    ...
    def generate_report(
        ...
        try:
            ...
            ReportManager.update_progress(
                ...
                "pending",
                0,
                "Initializing report..." if self.locale != "zh" else "初始化报告...",
                completed_sections=[],
            )
            ...
            ReportManager.update_progress(
                ...
                "planning",
                15,
                f"Outline complete, {len(outline.sections)} sections planned",
                completed_sections=[],
            )
            ReportManager.save_report(report)
            ...
            for i, section in enumerate(outline.sections):
                ...
                _gen_msg = (
                    ...
                    if self.locale != "zh"
                    else f"正在生成章节: {section.title} ({section_num}/{total_sections})"
                )
                ReportManager.update_progress(
                    report_id,
                    "generating",
            ...
            if progress_callback:
                progress_callback("generating", 95, _assemble_msg)
    ...
    def chat(
        ...
        system_prompt = _chat_tpl.format(
            ...
            if report_content
            else ("(No report yet)" if self.locale != "zh" else "（暂无报告）"),
            tools_description=self._get_tools_description(),
        )
        ...
        for iteration in range(max_iterations):
            ...
            messages.append(
                {"role": "user", "content": observation + p.CHAT_OBSERVATION_SUFFIX}
            )
