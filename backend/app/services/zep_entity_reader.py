...
class ZepEntityReader:
    ...
    def filter_defined_entities(
        ...
        allow_generic_entity_label: bool = False,
    ) -> FilteredEntities:
        """
        筛选出符合预定义实体类型的节点
        ...
        Args:
            ...
            allow_generic_entity_label: 当找不到自定义标签时，是否允许仅有默认标签的节点作为通用实体
        ...
        for node in all_nodes:
            ...
            if not custom_labels:
                # 只有默认标签时，默认跳过；在降级模式下作为通用实体保留
                if allow_generic_entity_label and any(
                    l in ["Entity", "Node"] for l in labels
                ):
