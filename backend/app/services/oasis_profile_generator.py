...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        comprehensive_query = (
            ...
            if self.locale != "zh"
            else f"关于{entity_name}的所有信息、活动、事件、关系和背景"
        )
        ...
        try:
            ...
            if results["facts"]:
                context_parts.append(
                    ("Facts:\n" if self.locale != "zh" else "事实信息:\n")
                    + "\n".join(f"- {f}" for f in results["facts"][:20])
                )
            if results["node_summaries"]:
                context_parts.append(
                    ("Related entities:\n" if self.locale != "zh" else "相关实体:\n")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        if entity.related_edges:
            ...
            for edge in entity.related_edges:  # 不限制数量
                ...
                if fact:
                    ...
                    _related_label = (
                        "related entity" if self.locale != "zh" else "相关实体"
                    )
                    if direction == "outgoing":
                        relationships.append(
                            f"- {entity.name} --[{edge_name}]--> ({_related_label})"
                        )
                    else:
                        relationships.append(
                            f"- ({_related_label}) --[{edge_name}]--> {entity.name}"
                        )
        ...
        if zep_results.get("node_summaries"):
            context_parts.append(
                (
                    "### Related Nodes Retrieved via Zep\n"
                    if self.locale != "zh"
                    else "### Zep检索到的相关节点\n"
                )
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        return p.PROFILE_SYSTEM_PROMPT
    ...
    def _build_individual_persona_prompt(
        ...
        """构建个人实体的详细人设提示词"""
        p = get_prompts(self.locale)
        _none_label = "None" if self.locale != "zh" else "无"
        _no_ctx_label = (
            "No additional context" if self.locale != "zh" else "无额外上下文"
        )
        ...
        context_str = context[:3000] if context else _no_ctx_label

        return p.PROFILE_INDIVIDUAL_USER_PROMPT_TEMPLATE.format(
    ...
    def _build_group_persona_prompt(
        ...
        return p.PROFILE_GROUP_USER_PROMPT_TEMPLATE.format(
            entity_name=entity_name,
            entity_type=entity_type,
            entity_summary=entity_summary,
            attrs_str=attrs_str,
    ...
    def _generate_profile_rule_based(
        ...
        if entity_type_lower in ["student", "alumni"]:
            ...
            return {
                ...
                "mbti": "ISTJ",  # 机构风格：严谨保守
                "country": "China" if self.locale != "zh" else "中国",
                "profession": "Media",
                "interested_topics": [
                    "General News",
                    "Current Events",
    ...
    def generate_profiles_from_entities(
        ...
        print(
            ...
            if self.locale != "zh"
            else f"开始生成Agent人设 - 共 {total} 个实体，并行数: {parallel_count}"
        )
        print(f"{'=' * 60}\n")
                    ...
                    if progress_callback:
                        progress_callback(
                            ...
                            (
                                ...
                                if self.locale != "zh"
                                else f"已完成 {current}/{total}: {entity.name}（{entity_type}）"
                            ),
                        )
        ...
        _done_count = len([p for p in profiles if p])
        print(f"\n{'=' * 60}")
        print(
            f"Profile generation complete! Generated {_done_count} Agents"
            if self.locale != "zh"
            else f"人设生成完成！共生成 {_done_count} 个Agent"
        )
        print(f"{'=' * 60}\n")
    ...
    def _print_generated_profile(
        ...
        topics_str = (
            ...
            else ("None" if self.locale != "zh" else "无")
        )
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                ...
                if profile.country
                else ("China" if self.locale != "zh" else "中国"),
            }
