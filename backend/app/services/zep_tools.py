...
class SearchResult:
    ...
    def to_text(self, locale: str = "en") -> str:
        """转换为文本格式，供LLM理解"""
        if locale == "zh":
            search_query_label = "搜索查询"
            found_label = f"找到 {self.total_count} 条相关信息"
            facts_label = "### 相关事实:"
        else:
            search_query_label = "Search Query"
            found_label = f"Found {self.total_count} items of related information"
            facts_label = "### Related Facts:"

        text_parts = [f"{search_query_label}: {self.query}", found_label]
...
class NodeInfo:
    ...
    def to_text(self, locale: str = "en") -> str:
        ...
        entity_type = next(
            (l for l in self.labels if l not in ["Entity", "Node"]), unknown_type
        )
        return f"{entity_label}: {self.name} ({type_label}: {entity_type})\n{summary_label}: {self.summary}"
...
class EdgeInfo:
    ...
    def to_text(self, include_temporal: bool = False, locale: str = "en") -> str:
        """转换为文本格式"""
        if locale == "zh":
            relation_label = "关系"
            fact_label = "事实"
            ...
            until_now_label = "Present"
            expired_label = "Expired"

        source = self.source_node_name or self.source_node_uuid[:8]
        target = self.target_node_name or self.target_node_uuid[:8]
        base_text = f"{relation_label}: {source} --[{self.name}]--> {target}\n{fact_label}: {self.fact}"

        if include_temporal:
            valid_at = self.valid_at or unknown_label
            invalid_at = self.invalid_at or until_now_label
            base_text += f"\n{validity_label}: {valid_at} - {invalid_at}"
            if self.expired_at:
                base_text += f" ({expired_label}: {self.expired_at})"
    ...
    def to_text(self, locale: str = "en") -> str:
        ...
        if locale == "zh":
            L = {
                ...
                "query": "分析问题",
                "scenario": "预测场景",
                "stats_heading": "### 预测数据统计",
                "facts_stat": "相关预测事实",
                "entities_stat": "涉及实体",
                "relations_stat": "关系链",
                "facts_unit": "条",
                "entities_unit": "个",
                "sub_queries_heading": "### 分析的子问题",
                "key_facts_heading": "### 【关键事实】(请在报告中引用这些原文)",
                "core_entities_heading": "### 【核心实体】",
                "relations_heading": "### 【关系链】",
                "unknown_name": "未知",
                "default_type": "实体",
                "summary_label": "摘要",
                "related_facts_label": "相关事实",
            }
        else:
            L = {
                "heading": "## Future Prediction Deep Analysis",
                "query": "Analysis Question",
                "scenario": "Prediction Scenario",
                "stats_heading": "### Prediction Data Statistics",
                "facts_stat": "Related Prediction Facts",
                "entities_stat": "Involved Entities",
                "relations_stat": "Relationship Chains",
                "facts_unit": " items",
                "entities_unit": "",
                "sub_queries_heading": "### Sub-Questions Analyzed",
                "key_facts_heading": "### [Key Facts] (please cite these verbatim in the report)",
                "core_entities_heading": "### [Core Entities]",
                "relations_heading": "### [Relationship Chains]",
                "unknown_name": "Unknown",
                "default_type": "Entity",
                "summary_label": "Summary",
                "related_facts_label": "Related Facts",
            }

        text_parts = [
            L["heading"],
            f"{L['query']}: {self.query}",
            f"{L['scenario']}: {self.simulation_requirement}",
            f"\n{L['stats_heading']}",
            f"- {L['facts_stat']}: {self.total_facts}{L['facts_unit']}",
            f"- {L['entities_stat']}: {self.total_entities}{L['entities_unit']}",
            f"- {L['relations_stat']}: {self.total_relationships}{L['facts_unit']}",
        ]

        # 子问题
        if self.sub_queries:
            text_parts.append(f"\n{L['sub_queries_heading']}")
        ...
        if self.entity_insights:
            text_parts.append(f"\n{L['core_entities_heading']}")
            for entity in self.entity_insights:
                text_parts.append(
                    f"- **{entity.get('name', L['unknown_name'])}** ({entity.get('type', L['default_type'])})"
                )
                if entity.get("summary"):
                    text_parts.append(
                        f'  {L["summary_label"]}: "{entity.get("summary")}"'
                    )
                if entity.get("related_facts"):
                    text_parts.append(
                        f"  {L['related_facts_label']}: {len(entity.get('related_facts', []))}{L['facts_unit']}"
                    )
...
class PanoramaResult:
    ...
    def to_text(self, locale: str = "en") -> str:
        ...
        if locale == "zh":
            L = {
                "heading": "## 广度搜索结果（未来全景视图）",
                "query_label": "查询",
                "stats_heading": "### 统计信息",
                "total_nodes": "总节点数",
                "total_edges": "总边数",
                "active_facts_stat": "当前有效事实",
                "historical_facts_stat": "历史/过期事实",
                "unit": "条",
                "active_heading": "### 【当前有效事实】(模拟结果原文)",
                "historical_heading": "### 【历史/过期事实】(演变过程记录)",
                "entities_heading": "### 【涉及实体】",
                "default_type": "实体",
            }
        else:
            L = {
                "heading": "## Panorama Search Results (Future Panoramic View)",
                "query_label": "Query",
                "stats_heading": "### Statistics",
                "total_nodes": "Total Nodes",
                "total_edges": "Total Edges",
                "active_facts_stat": "Currently Valid Facts",
                "historical_facts_stat": "Historical/Expired Facts",
                "unit": " items",
                "active_heading": "### [Currently Valid Facts] (Simulation result verbatim)",
                "historical_heading": "### [Historical/Expired Facts] (Evolution records)",
                "entities_heading": "### [Involved Entities]",
                "default_type": "Entity",
            }

        text_parts = [
            L["heading"],
            f"{L['query_label']}: {self.query}",
            f"\n{L['stats_heading']}",
            f"- {L['total_nodes']}: {self.total_nodes}",
            f"- {L['total_edges']}: {self.total_edges}",
            f"- {L['active_facts_stat']}: {self.active_count}{L['unit']}",
            f"- {L['historical_facts_stat']}: {self.historical_count}{L['unit']}",
        ]
...
class AgentInterview:
    ...
    def to_text(self, locale: str = "en") -> str:
        if locale == "zh":
            ...
            bio_label = "Bio"
            quotes_label = "Key Quotes"

        text = f"**{self.agent_name}** ({self.agent_role})\n"
        # 显示完整的agent_bio，不截断
        text += f"_{bio_label}: {self.agent_bio}_\n\n"
    ...
    def to_text(self, locale: str = "en") -> str:
        ...
        if locale == "zh":
            L = {
                ...
                "topic_label": "**采访主题:**",
                "count_label": "**采访人数:**",
                "count_suffix": "位模拟Agent",
                "selection_heading": "### 采访对象选择理由",
                "auto_select": "（自动选择）",
                "records_heading": "### 采访实录",
                "interview_prefix": "#### 采访 #",
                "no_records": "（无采访记录）",
                "summary_heading": "### 采访摘要与核心观点",
                "no_summary": "（无摘要）",
            }
        else:
            L = {
                "heading": "## In-Depth Interview Report",
                "topic_label": "**Interview Topic:**",
                "count_label": "**Interview Count:**",
                "count_suffix": "simulated Agents",
                "selection_heading": "### Interview Subject Selection Reasons",
                "auto_select": "(Auto-selected)",
                "records_heading": "### Interview Records",
                "interview_prefix": "#### Interview #",
                "no_records": "(No interview records)",
                "summary_heading": "### Interview Summary & Core Viewpoints",
                "no_summary": "(No summary)",
            }

        text_parts = [
            L["heading"],
            f"{L['topic_label']} {self.interview_topic}",
            f"{L['count_label']} {self.interviewed_count} / {self.total_agents} {L['count_suffix']}",
            f"\n{L['selection_heading']}",
            self.selection_reasoning or L["auto_select"],
            "\n---",
            f"\n{L['records_heading']}",
        ]
        ...
        if self.interviews:
            for i, interview in enumerate(self.interviews, 1):
                text_parts.append(
                    f"\n{L['interview_prefix']}{i}: {interview.agent_name}"
                )
                text_parts.append(interview.to_text(locale=locale))
                text_parts.append("\n---")
        else:
            text_parts.append(f"{L['no_records']}\n\n---")

        text_parts.append(f"\n{L['summary_heading']}")
        text_parts.append(self.summary or L["no_summary"])
...
class ZepToolsService:
    ...
    def get_graph_statistics(self, graph_id: str) -> Dict[str, Any]:
        ...
        Returns:
            统计信息
        """
        logger.info(f"Fetching graph stats for {graph_id}...")
    ...
    def _generate_sub_queries(
        ...
        system_prompt = p.SUB_QUESTION_SYSTEM_PROMPT

        context_line = f"报告上下文：{report_context[:500]}" if report_context else ""
        user_prompt = p.SUB_QUESTION_USER_PROMPT_TEMPLATE.format(
            requirement=simulation_requirement,
            simulation_requirement=simulation_requirement,
            context=context_line,
            report_context_line=context_line,
        ...
        try:
            ...
            # 降级：返回基于原问题的变体
            fallbacks = [
                t.format(query=query) for t in p.SUB_QUESTION_FALLBACK_TEMPLATES
            ]
    ...
    def interview_agents(
        ...
        optimized_prompt = f"{p.INTERVIEW_PROMPT_PREFIX}{combined_prompt}"
        ...
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                if locale == "zh":
                    no_reply = "（该平台未获得回复）"
                    twitter_label = "【Twitter平台回答】"
                    reddit_label = "【Reddit平台回答】"
                else:
                    no_reply = "(No response from this platform)"
                    twitter_label = "[Twitter Response]"
                    reddit_label = "[Reddit Response]"
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = p.INTERVIEW_SELECT_SYSTEM_PROMPT

        user_prompt = p.INTERVIEW_SELECT_USER_PROMPT_TEMPLATE.format(
            interview_requirement=interview_requirement,
            simulation_requirement=simulation_requirement
            if simulation_requirement
            else "未提供",
            agent_count=len(agent_summaries),
            agent_summaries_json=json.dumps(
                agent_summaries, ensure_ascii=False, indent=2
            ),
            max_agents=max_agents,
        )
    ...
    def _generate_interview_questions(
        ...
        system_prompt = p.INTERVIEW_QUESTION_SYSTEM_PROMPT

        user_prompt = p.INTERVIEW_QUESTION_USER_PROMPT_TEMPLATE.format(
            interview_requirement=interview_requirement,
            simulation_requirement=simulation_requirement
            if simulation_requirement
            else "未提供",
        ...
        try:
            ...
            fallbacks = [
                t.format(interview_requirement=interview_requirement)
                for t in p.INTERVIEW_QUESTION_FALLBACK_TEMPLATES
            ]
            return fallbacks
    ...
    def _generate_interview_summary(
        ...
        system_prompt = p.INTERVIEW_SUMMARY_SYSTEM_PROMPT

        user_prompt = p.INTERVIEW_SUMMARY_USER_PROMPT_TEMPLATE.format(
            interview_requirement=interview_requirement,
