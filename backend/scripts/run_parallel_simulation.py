...
_provider_index = 0  # Round-robin index for LLM_PROVIDERS


def _get_provider_from_registry():
    """Pick the next provider from LLM_PROVIDERS JSON (round-robin)."""
    ...
    try:
        ...
        providers = json.loads(providers_json.strip().rstrip(","))
        if not providers:
            return None, None, None
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    """
    Create an LLM model for OASIS simulation.

    Supports multiple config sources (in priority order):
    1. Boost config: LLM_BOOST_API_KEY / LLM_BOOST_BASE_URL / LLM_BOOST_MODEL_NAME
    2. Legacy config: LLM_API_KEY / LLM_BASE_URL / LLM_MODEL_NAME
    3. Multi-provider: LLM_PROVIDERS JSON array (round-robin)
    """
    # Check boost config
    ...
    if use_boost and has_boost_config:
        ...
        # Try legacy single-provider env vars first
        llm_api_key = os.environ.get("LLM_API_KEY", "")
        llm_base_url = os.environ.get("LLM_BASE_URL", "")
        llm_model = os.environ.get("LLM_MODEL_NAME", "")
        config_label = "[LLM]"

        # If legacy not set, try LLM_PROVIDERS round-robin
        if not llm_api_key:
            rr_key, rr_url, rr_model = _get_provider_from_registry()
            if rr_key:
                llm_api_key = rr_key
                llm_base_url = rr_url or llm_base_url
                llm_model = rr_model or llm_model
                config_label = "[LLM Provider RR]"
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError(
            "No API key configured. Set LLM_API_KEY or LLM_PROVIDERS in .env"
        )
