        ...
        <div class="card-content">
          ...
          <div v-if="simulationId" class="info-card">
            ...
            <div class="info-row">
              ...
              <span class="info-value mono">{{ taskId || $t('step2.asyncComplete') }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.agentGen') }}</span>
          </div>
          <div class="step-status">
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-block-header">
                ...
                <span class="config-block-badge">{{ simulationConfig.agent_configs?.length || 0 }} {{ $t('common.agents') }}</span>
              </div>
              <div class="agents-cards">
                ...
                >
                  ...
                  <div class="agent-params">
                    ...
                    <div class="param-group">
                      <div class="param-item">
                        <span class="param-label">{{ $t('step2.activeness') }}</span>
            ...
            <div class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">{{ $t('step2.recoAlgo') }}</span>
              </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        <div class="card-header">
          ...
          <div class="step-status">
            ...
            <span v-else-if="phase === 3" class="badge processing">{{ $t('common.processing') }}</span>
            <span v-else class="badge pending">{{ $t('common.waiting') }}</span>
          </div>
        </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.readyToSim') }}</span>
          </div>
          <div class="step-status">
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">MiroFish <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> {{ $t('common.hours') }} / <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> {{ $t('common.minutes') }} per round</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>{{ $t('step2.agentScaleHint', { minutes: Math.round(customMaxRounds * 0.6) }) }}</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        {{ $t('step2.agentScaleHint', { minutes: Math.round(autoGeneratedRounds * 0.6) }) }}
                      </span>
                    </div>
                    <div class="auto-desc">
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-section">
            <span class="section-label">{{ $t('step2.bioLabel') }}</span>
            <p class="section-bio">{{ selectedProfile.bio || $t('step2.noBio') }}</p>
          </div>

          <!-- 关注话题 -->
          <div class="modal-section" v-if="selectedProfile.interested_topics?.length">
            <span class="section-label">{{ $t('step2.topicsLabel') }}</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            <span class="section-label">{{ $t('step2.backgroundLabel') }}</span>
...
watch(currentStage, (newStage) => {
  if (newStage === '生成Agent人设' || newStage === 'generating_profiles') {
    ...
    // 进入配置生成阶段，开始轮询配置
    if (!configTimer) {
      addLog(t('logs.generatingDualConfig'))
      startConfigPolling()
    }
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(t('logs.startSimCustomRounds', { rounds: customMaxRounds.value }))
  } else {
    // 用户选择保持自动生成的轮数，不传递 max_rounds 参数
    addLog(t('logs.startSimAutoRounds', { rounds: autoGeneratedRounds.value }))
  }
...
const startPrepareSimulation = async () => {
  if (!props.simulationId) {
    addLog(t('logs.errorMissingSimId'))
  ...
  // 标记第一步完成，开始第二步
  phase.value = 1
  addLog(t('logs.simCreated', { id: props.simulationId }))
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog(t('logs.alreadyPrepared'))
        await loadPreparedData()
        return
      }
      
      taskId.value = res.data.task_id
      addLog(t('logs.prepareTaskStarted'))
      addLog(`  └─ ${t('logs.taskId', { id: res.data.task_id })}`)
      
      // 立即设置预期Agent总数（从prepare接口返回值获取）
      if (res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_entities_count
        addLog(t('logs.entitiesRead', { count: res.data.expected_entities_count }))
        if (res.data.entity_types && res.data.entity_types.length > 0) {
          addLog(`  └─ ${t('logs.entityTypes', { types: res.data.entity_types.join(', ') })}`)
        }
      }
      ...
      addLog(t('logs.prepareFailed', { error: res.error || t('common.unknownError') }))
      emit('update-status', 'error')
    }
  } catch (err) {
    addLog(t('logs.prepareError', { error: err.message }))
    emit('update-status', 'error')
  }
}
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        addLog(`✓ ${t('logs.prepareComplete')}`)
        stopPolling()
        stopProfilesPolling()
        await loadPreparedData()
      } else if (data.status === 'failed') {
        addLog(`✗ ${t('logs.prepareFailedStatus', { error: data.error || t('common.unknownError') })}`)
        stopPolling()
        stopProfilesPolling()
      }
    }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        if (currentCount === 1) {
          addLog(t('logs.generatingAgentPersonas'))
        }
        addLog(`→ ${t('logs.personaGenerated', { current: currentCount, total, name: profileName, type: latestProfile?.profession || t('logs.unknownProfession') })}`)
        
        // 如果全部生成完成
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`✓ ${t('logs.allPersonasComplete', { count: currentCount })}`)
        }
      }
    }
  } catch (err) {
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          addLog(t('logs.generatingProfileConfig'))
        } else if (data.generation_stage === 'generating_config') {
          addLog(t('logs.callingLlmConfig'))
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        addLog(`✓ ${t('logs.configGenComplete')}`)
        
        // 显示详细配置摘要
        if (data.summary) {
          addLog(`  ├─ ${t('logs.agentCount', { count: data.summary.total_agents })}`)
          addLog(`  ├─ ${t('logs.simDuration', { hours: data.summary.simulation_hours })}`)
          addLog(`  ├─ ${t('logs.initialPosts', { count: data.summary.initial_posts_count })}`)
          addLog(`  ├─ ${t('logs.hotTopics', { count: data.summary.hot_topics_count })}`)
        ...
        if (data.config.time_config) {
          ...
          addLog(t('logs.timeConfigDetail', { minutes: tc.minutes_per_round, rounds: Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round) }))
        }
        
        // 显示事件配置
        if (data.config.event_config?.narrative_direction) {
          const narrative = data.config.event_config.narrative_direction
          addLog(t('logs.narrativeDirection', { text: narrative.length > 50 ? narrative.substring(0, 50) + '...' : narrative }))
        }
        
        stopConfigPolling()
        phase.value = 4
        addLog(`✓ ${t('logs.envSetupComplete')}`)
...
const loadPreparedData = async () => {
  ...
  addLog(t('logs.loadedAgentPersonas', { count: profiles.value.length }))
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog(`✓ ${t('logs.configLoadSuccess')}`)
        
        // 显示详细配置摘要
        if (res.data.summary) {
          addLog(`  ├─ ${t('logs.agentCount', { count: res.data.summary.total_agents })}`)
          addLog(`  ├─ ${t('logs.simDuration', { hours: res.data.summary.simulation_hours })}`)
          addLog(`  └─ ${t('logs.initialPosts', { count: res.data.summary.initial_posts_count })}`)
        }
        ...
        // 配置尚未生成，开始轮询
        addLog(t('logs.configGenerating'))
        startConfigPolling()
      }
    }
  } catch (err) {
    addLog(t('logs.configLoadFailed', { error: err.message }))
    emit('update-status', 'error')
  }
}
