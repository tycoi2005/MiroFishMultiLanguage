          ...
          <div class="action-bar-tabs">
            ...
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : $t('step5.chatAgent') }}</span>
...
const sendMessage = async () => {
  ...
  try {
    ...
    chatHistory.value.push({
      role: 'assistant',
      content: `${t('step5.requestFailed')}: ${err.message}`,
      timestamp: new Date().toISOString()
    })
  } finally {
...
const sendToAgent = async (message) => {
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? t('step5.you') : selectedAgent.value?.username || 'Agent'}：${msg.content}`)
      .join('\n')
    prompt = `${t('step5.chatHistoryPrefix')}\n${historyContext}\n\n${t('step5.chatNewQuestion')}${message}`
  }
