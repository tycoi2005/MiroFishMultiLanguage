...
const doStartSimulation = async () => {
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(t('logs.setMaxRounds', { rounds: props.maxRounds }))
    }
    
    addLog(t('logs.dynamicGraphMode'))
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog(`✓ ${t('logs.oldLogsCleared')}`)
      }
      addLog(`✓ ${t('logs.simEngineStarted')}`)
      addLog(`  ├─ ${t('logs.pid', { pid: res.data.process_pid || '-' })}`)
      ...
      startError.value = res.error || t('logs.startFailed', { error: '' })
      addLog(`✗ ${t('logs.startFailed', { error: res.error || t('common.unknownError') })}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ ${t('logs.startError', { error: err.message })}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(t('logs.stopFailed', { error: res.error || t('common.unknownError') }))
    }
  } catch (err) {
    addLog(t('logs.stopError', { error: err.message }))
  } finally {
    isStopping.value = false
  }
}
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog(`✓ ${t('logs.allPlatformsComplete')}`)
        }
        addLog(`✓ ${t('logs.simComplete')}`)
        phase.value = 2
        stopPolling()
        emit('update-status', 'completed')
      }
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog(t('logs.reportAlreadySent'))
    return
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ ${t('logs.reportTaskStarted', { id: reportId })}`)
      
      // 跳转到报告页面
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ ${t('logs.reportStartFailed', { error: res.error || t('common.unknownError') })}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ ${t('logs.reportStartError', { error: err.message })}`)
    isGeneratingReport.value = false
  }
}
