...
const emit = defineEmits(['add-log', 'update-status', 'update-report-id'])
...
const regenerateReport = async () => {
  ...
  try {
    // Stop polling old report first to avoid mixing logs from two report IDs
    stopPolling()
    ...
    if (res?.success && res?.data?.report_id) {
      ...
      // Tell parent view to update report state
      emit('update-report-id', newReportId)

      // Ensure URL immediately follows the new report ID
      if (newReportId !== props.reportId) {
        await router.push({ name: 'Report', params: { reportId: newReportId } })
      }

      // Wait for Vue to process the route/prop update
      await nextTick()
    } else {
      throw new Error('Failed to start new report generation')
    }
