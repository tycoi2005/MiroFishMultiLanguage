...
MiroFish 现已支持**故事模式** —— 除了预测未来，你还可以用它从种子材料中**生成创意叙事**。
...
| 模式 | 功能说明 |
|------|---------|
| **预测模式**（默认） | 分析种子数据，生成分析性预测报告 |
| **故事模式** | 使用同样的模拟引擎，但生成创意小说 —— 支持小说体或剧本格式 |

### 故事输出格式

- **小说**：丰富的散文叙事，包含生动的场景描写、真实的人物对话、内心独白和完整的叙事弧线
- **剧本**：专业剧本格式，包含内景/外景标题、角色名大写标注、舞台指示和视觉化叙事

### 使用示例

上传一份世界构建文档（例如：包含自定义角色的哈利波特宇宙种子文件），选择**故事模式**，然后输入故事前提：

> *"从Derek Thorne的视角写一个故事——一个把枪偷偷带进霍格沃茨的麻瓜出身男孩"*

MiroFish 将模拟角色互动，然后生成多章节叙事，角色会根据自身性格和人际关系自主行动。
...
模拟Agent以**虚构角色**而非社交媒体用户的身份行动，Report Agent 撰写**叙事章节**而非分析报告。
